...
@section('content')
    <div class="h-[72.7vh] flex items-center justify-center bg-neutral-700 text-white">
        <form action="/admin_main_cards_2_edit/{{ $cards->id }}" method="post" class="flex flex-col items-center">
            @method('patch')
            @csrf
            <table>
                <tr class="mb-3 flex justify-between gap-10 items-center">
                    <td>
                        <label for="video" class="form-label text-xl">VIDEO</label>
                    </td>
                    <td>
                        <input type="text" name="video" id="video" class="w-[500px] form-control bg-neutral-700"
                            value="{{ $cards->video }}">
                    </td>
                </tr>
            </table>
