...
@section('content')
    <div class="min-h-screen pt-[200px] overflow-x-auto flex items-center justify-center bg-neutral-700 text-white">
        <form action="/admin_games_cards_edit/{{ $cards->id }}" method="post" class="flex flex-col items-center">
            @method('patch')
            @csrf
            <table>
                <tr class="mb-3 flex justify-between gap-5 items-center">
                    <td>
                        <label for="header" class="form-label text-xl">HEADER</label>
                    </td>
                    <td>
                        <input type="text" name="header" id="header" class="w-[500px] form-control bg-neutral-700"
                            value="{{ $cards->header }}">
                    </td>
                </tr>
                <tr class="mb-3 flex justify-between gap-5 items-center">
